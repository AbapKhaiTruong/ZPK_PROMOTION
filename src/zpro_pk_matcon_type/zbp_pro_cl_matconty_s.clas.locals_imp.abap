...
CLASS LHC_RAP_TDAT_CTS IMPLEMENTATION.
  METHOD GET.
    result = mbc_cp_api=>rap_tdat_cts( tdat_name = 'ZPRO_MATCONTY'
                                       table_entity_relations = VALUE #(
                                         ( entity = 'ConditionType' table = 'ZPRO_TB_MATCONTY' )
                                       ) ) ##NO_TEXT.
  ENDMETHOD.
ENDCLASS.
CLASS LHC_ZPRO_I_MATCONTY_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
...
CLASS LHC_ZPRO_I_MATCONTY_S IMPLEMENTATION.
  METHOD GET_INSTANCE_FEATURES.
    ...
    ENDIF.
    READ ENTITIES OF ZPRO_I_MATCONTY_S IN LOCAL MODE
  ...
  METHOD SELECTCUSTOMIZINGTRANSPTREQ.
    MODIFY ENTITIES OF ZPRO_I_MATCONTY_S IN LOCAL MODE
      ENTITY ConditionTypeAll
  ...
  METHOD GET_GLOBAL_AUTHORIZATIONS.
    AUTHORITY-CHECK OBJECT 'S_TABU_NAM' ID 'TABLE' FIELD 'ZPRO_I_MATCONTY' ID 'ACTVT' FIELD '02'.
  ...
  ENDMETHOD.
ENDCLASS.
CLASS LSC_ZPRO_I_MATCONTY_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_SAVER.
  PROTECTED SECTION.
    METHODS:
...
CLASS LSC_ZPRO_I_MATCONTY_S IMPLEMENTATION.
  METHOD SAVE_MODIFIED.
    ...
    IF all-TransportRequestID IS NOT INITIAL.
      lhc_rap_tdat_cts=>get( )->record_changes(
                                  ...
                                  create            = REF #( create )
                                  update            = REF #( update )
                                  delete            = REF #( delete ) )->update_last_changed_date_time( view_entity_name   = 'ZPRO_I_MATCONTY'
                                                                                                        maintenance_object = 'ZPRO_MATCONTY' ).
    ENDIF.
  ENDMETHOD.
  METHOD CLEANUP_FINALIZE ##NEEDED.
  ENDMETHOD.
ENDCLASS.
CLASS LHC_ZPRO_I_MATCONTY DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
...
CLASS LHC_ZPRO_I_MATCONTY IMPLEMENTATION.
  ...
  METHOD COPYCONDITIONTYPE.
    DATA new_ConditionType TYPE TABLE FOR CREATE ZPRO_I_MATCONTY_S\_ConditionType.
  ...
  METHOD VALIDATETRANSPORTREQUEST.
    DATA change TYPE REQUEST FOR CHANGE ZPRO_I_MATCONTY_S.
    IF keys_ConditionType IS NOT INITIAL.
      DATA(is_draft) = keys_ConditionType[ 1 ]-%IS_DRAFT.
    ELSE.
      RETURN.
    ENDIF.
    READ ENTITY IN LOCAL MODE ZPRO_I_MATCONTY_S
    ...
    lhc_rap_tdat_cts=>get( )->validate_all_changes(
                                ...
                                table_validation_keys = VALUE #(
                                                          ( table = 'ZPRO_TB_MATCONTY' keys = REF #( keys_ConditionType ) )
                                                               )
